...
regex="(\/queue\/item\/([0-9]+))"
#Does the returned value contain 201 Created
if [[ $TRIGGERJOBJSON == *"201"* ]]; then
...
if [[ $TRIGGERJOBJSON =~ $regex ]]; then
  ...
  echo "--------------------------------------"
  QUEUEURL="$urlOfJenkinsServer${BASH_REMATCH[1]}" ;
  if [[ $verbose == true ]]; then
    echo $QUEUEURL ; 
    echo "--------------------------------------"
  fi
...
if [[ $BUILDJSON =~ $regex ]]; then
  if [[ $verbose == true ]]; then
    ...
    echo "build URL: ${urlOfJenkinsServer}/job/${jenkinsJobName}/${BASH_REMATCH[2]}" ;
  fi
  echo "Job URL retrieved"
  echo "--------------------------------------"
  BUILDURL="${urlOfJenkinsServer}/job/${jenkinsJobName}/${BASH_REMATCH[2]}";
